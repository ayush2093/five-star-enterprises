...
const Profile = ({ user, updateUser }) => {
  ...
  useEffect(() => {
    const fetchDesigns = async () => {
      ...
      try {
        ...
        let localDesigns = [];
        const saved = localStorage.getItem('fivestar_saved_designs');
        if (saved) {
  ...
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === 'fivestar_saved_designs') {
        const saved = localStorage.getItem('fivestar_saved_designs');
        if (saved) {
          const parsed = JSON.parse(saved);
          const filtered = user?._id || user?.id
  ...
  const handleShareDesign = useCallback((design) => {
    const shareText = `Check out my custom fleet configuration: ${design.name}`;
    if (navigator.share) {
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
      <div>
        ...
        <p className="text-gray-600 text-sm sm:text-base">
          {designCount > 0
            ? `You have ${designCount} custom vehicle config${designCount !== 1 ? 's' : ''}`
            : 'Start your travel plans by customizing your first vehicle'}
        </p>
      </div>
      <button
...
const EmptyDesignsState = React.memo(() => (
  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-2xl p-10 sm:p-12 text-center">
    ...
    <p className="text-gray-600 mb-8 max-w-md mx-auto text-sm sm:text-base">Start your journey by configuring your first custom ride. It's easy and fun!</p>
    <button
      onClick={() => window.location.href = '/designer'}
